        ...
        <tbody id="supplierTableBody">
            @foreach($suppliers as $supplier)
                <tr id="supplier-row-{{ $supplier->id }}">
                    ...
                    <td class="px-4 py-2">
                        <a href="{{ url('/edit-supplier/'.$supplier->id) }}" 
...
<script>
    ...
    function searchSupplier() {
        const searchValue = document.getElementById('supplier-search').value.toLowerCase();
        const rows = document.querySelectorAll('#supplierTableBody tr');

        rows.forEach(row => {
            const supplierIdCell = row.querySelector('td:nth-child(1)'); // Get the first column (Supplier ID)
            const supplierNameCell = row.querySelector('td:nth-child(2)'); // Get the second column (Supplier Name)

            const supplierId = supplierIdCell ? supplierIdCell.textContent.toLowerCase() : '';
            const supplierName = supplierNameCell ? supplierNameCell.textContent.toLowerCase() : '';

            // If the Supplier ID or Supplier Name matches the search term, show the row, otherwise hide it
