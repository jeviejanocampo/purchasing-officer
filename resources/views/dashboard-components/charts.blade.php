...
<script>
    ...
    const productCosts = @json($productCosts); // This will now have data passed from the controller

    const labels = productCosts.map(item => item.name);  // Extract product names
    const data = productCosts.map(item => item.cost);    // Extract product costs
